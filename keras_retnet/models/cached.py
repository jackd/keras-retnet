...
class _Identity(keras.Operation):
    def call(self, x):
        return x
...
def _call_to_func(operation: keras.Operation, args, kwargs, static_kwargs, name):
    ...
    # identity mapping removes bugs when layers return an input argument unchanged
    outputs = tree.map_structure(_identity, outputs)
