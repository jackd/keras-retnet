...
from keras_retnet.models.retnet.causal_lm import RetnetCausalLM


class RwkvCausalLMTest(unittest.TestCase):
    def test_generation_consistent(self):
        lm = RetnetCausalLM.from_preset("retnet-base", load_weights=False)
        start = lm.preprocessor.tokenizer.start_token_id

        inputs = {
            "token_ids": keras.ops.convert_to_tensor([[start, 4, 3, 3, 3]], "int32"),
...
if __name__ == "__main__":
    # unittest.main()
    RwkvCausalLMTest().test_generation_consistent()
